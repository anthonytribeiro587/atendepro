  ...
  try {
    ...
    const { data: business, error: businessError } = await supabase
      ...
      .select(`
        ...
        evolution_template_confirmation
      `)
    ...
    if (client?.phone) {
      const credentials: WhatsAppCredentials = business.evolution_enabled
        ? {
      ...
      const message = renderWhatsAppTemplate(
        business.evolution_template_confirmation,
        {
          cliente: client.name,
          ...
          link_agendamento: business.slug
            ? `${process.env.NEXT_PUBLIC_APP_URL ?? req.nextUrl.origin}/book/${business.slug}`
            : undefined,
        },
        DEFAULT_EVOLUTION_TEMPLATES.confirmation
      )

      results.whatsapp = await sendWhatsAppMessage(client.phone, message, credentials)
    }
